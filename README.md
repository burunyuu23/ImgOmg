...
7.1. Его остаётся лишь сжечь и просить богов о милости.
...
# Запуск.
  1. Каждый из микросервисов на бекенде поднимаются простым запуском main.py:
  - Под простым запуском main.py я понимаю нажатие на зелёный треугольник, если кто не понял.
  2. Фронт поднимать либо `npm run dev`, либо поднимаем контейнер (докерфайл вроде в папке с фронтом) и все работает (вроде бы).
  3. Дбдшку поднимать докер композом. Надеюсь сможете (я просто забыл как, через недельку добавлю как, надеюсь...)
